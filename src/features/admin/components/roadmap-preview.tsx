...
export function RoadmapPreview() {
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-primary)]">
      {/* Preview Banner */}
      <div className="sticky top-0 z-50 bg-amber-500/10 border-b border-amber-500/30 px-4 py-2.5 backdrop-blur-xs">
        <div className="container-main flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Eye className="w-4 h-4 text-yellow-600" />
            <span className="text-sm font-medium text-yellow-800">Preview Mode</span>
            <Eye className="w-4 h-4 text-amber-600 dark:text-amber-400" />
            <span className="text-sm font-medium text-amber-900 dark:text-amber-200">Preview Mode</span>
            {status !== 'published' && (
          ...
          <div className="flex items-center gap-3">
            <Link href={`/admin/roadmaps/edit?id=${id}`} className="text-xs font-medium text-amber-800 dark:text-amber-300 hover:text-amber-950 dark:hover:text-amber-100 underline">
              Edit
            </Link>
            <Link href="/admin/roadmaps" className="text-xs font-medium text-amber-800 dark:text-amber-300 hover:text-amber-950 dark:hover:text-amber-100 underline">
              Back to list
            </Link>
          </div>
        </div>
